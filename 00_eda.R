...
# ── OUTLIER REMOVAL for LR and SVM (IQR method on top-20 features) ───────────
# LR and SVM are sensitive — remove rows with outliers in any top-20 feature.
...
# ── Save prepared objects for model scripts ───────────────────────────────────
save(
  train_df, test_df,
  y_train, y_test,
  top20_eda, lr2_features,
  numeric_predictors,
  feature_ranking,
  file = "prepared_data.RData"
)
cat("Saved prepared_data.RData\n")
